...
#include "lidar_localization/mapping/back_end/lio_mapping_flow.hpp"
...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<LIOBackEndFlow> lio_mapping_flow_ptr = std::make_shared<LIOBackEndFlow>(
    ...
    while (ros::ok()) {
        ...
        lio_mapping_flow_ptr->Run();

        if (_need_optimize_map) {
            lio_mapping_flow_ptr->ForceOptimize();
            lio_mapping_flow_ptr->SaveOptimizedOdometry();
