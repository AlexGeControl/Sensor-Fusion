...
int main(int argc, char *argv[]) {
    ...
    ros::init(argc, argv, "imu_gnss_odo_filtering_node");
    ros::NodeHandle nh;

    std::shared_ptr<IMUGNSSOdoFilteringFlow> imu_gnss_odo_filtering_flow_ptr = std::make_shared<IMUGNSSOdoFilteringFlow>(nh);
    ...
    while (ros::ok()) {
        ...
        imu_gnss_odo_filtering_flow_ptr->Run();

        // save odometry estimations for evo evaluation:
        if ( _need_save_odometry && 
             imu_gnss_odo_filtering_flow_ptr->SaveOdometry() && 
             imu_gnss_odo_filtering_flow_ptr->SaveObservabilityAnalysis()
        ) {
