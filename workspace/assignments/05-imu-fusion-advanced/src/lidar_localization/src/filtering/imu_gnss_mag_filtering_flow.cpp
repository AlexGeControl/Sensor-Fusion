        ...
        if ( !HasInited() ) {
            ...
            if (  HasPosVelData() && ValidPosVelData() ) {
                if ( HasIMUData() ) {
                    while (
                        HasIMUData() && ValidIMUData() && 
                        current_imu_data_.time < current_pos_vel_data_.time
                    ) {
                        UpdateLocalization();
                    }
...
bool IMUGNSSMagFilteringFlow::InitLocalization(void) {
    ...
    gnss_pose_(2, 3) = current_pos_vel_data_.pos.z();

    Eigen::Vector3f init_vel = current_imu_data_.GetOrientationMatrix() * current_pos_vel_data_.vel;
    ...
    LOG(INFO) << "Init Error-State Kalman Filter with first GNSS-odo-IMU measurement" << std::endl;
...
bool IMUGNSSMagFilteringFlow::CorrectLocalization() {
    ...
    if ( 
        ...
        UpdateOdometry(current_pos_vel_data_.time);
