...
IMUGNSSOdoFiltering::IMUGNSSOdoFiltering() {   
    // load ROS config:
    InitWithConfig();
}

bool IMUGNSSOdoFiltering::Init(
...
bool IMUGNSSOdoFiltering::Update(
...
bool IMUGNSSOdoFiltering::Correct(
    const IMUData &imu_data,
...
void IMUGNSSOdoFiltering::GetOdometry(Eigen::Matrix4f &pose, Eigen::Vector3f &vel) {
    pose = init_pose_ * current_pose_;
    vel = init_pose_.block<3, 3>(0, 0) * current_vel_;
}

void IMUGNSSOdoFiltering::GetStandardDeviation(ESKFStd &eskf_std_msg) {
...
bool IMUGNSSOdoFiltering::InitWithConfig(void) {
    std::string config_file_path = WORK_SPACE_PATH + "/config/filtering/imu_gnss_odo_filtering.yaml";
...
bool IMUGNSSOdoFiltering::InitFusion(const YAML::Node& config_node) {
 ...
 */
bool IMUGNSSOdoFiltering::SetInitGNSS(const Eigen::Matrix4f& gnss_pose) {
...
bool IMUGNSSOdoFiltering::SetInitPose(const Eigen::Matrix4f& init_pose) {
